...
def test_l1_action_signing_order_matches():
    ...
    signature_mainnet = sign_l1_action(wallet, order_action, None, timestamp, True,)
    assert (
        signature_mainnet["r"]
    ...
    signature_testnet = sign_l1_action(wallet, order_action, None, timestamp, False,)
    assert (
        signature_testnet["r"]
...
def test_l1_action_signing_matches_with_vault():
    ...
    signature_mainnet = sign_l1_action(
        wallet, action, "0x1719884eb866cb12b2287399b15f7db5e7d775ea", 0, True
    )
    assert (
        signature_mainnet["r"]
    ...
    signature_testnet = sign_l1_action(
        wallet, action, "0x1719884eb866cb12b2287399b15f7db5e7d775ea", 0, False
    )
    assert (
        signature_testnet["r"]
...
def test_sign_usd_transfer_action():
    ...
    assert (
        signature["r"]
        == "0x283ca602ac69be536bd2272f050eddf8d250ed3eef083d1fc26989e57f891759"
    )
    assert (
        signature["s"]
        == "0x9bc743cf95042269236bc7f48c06ab8a6a9ee53e04f3336c6cfd1b22783aa74"
    )
...
def test_sign_withdraw_from_bridge_action():
    ...
    assert (
        signature["r"]
        == "0xd60816bf99a00645aa81b9ade23f03bf15994cd2c6d06fc3740a4c74530e36d9"
    )
    assert (
        signature["s"]
        == "0x4552f30419166a6e9d8dbd49b14aeef1e7606fe9e0caec8c0211608d79ce43a3"
    )
    assert signature["v"] == 28
...
def test_create_sub_account_action():
    ...
    assert (
        signature_mainnet["s"]
        == "0xb71f7d293af92d3772572afb8b102d167a7cef7473388286bc01f52a5c5b423"
    )
    assert signature_mainnet["v"] == 27
    signature_testnet = sign_l1_action(wallet, action, None, 0, False)
    assert (
    ...
    assert (
        signature_testnet["s"]
        == "0x53f1b8153a301c72ecc655b1c315d64e1dcea3ee58921fd7507e35818fcc1584"
    )
    assert signature_testnet["v"] == 28
...
def test_sub_account_transfer_action():
    ...
    assert (
        signature_testnet["s"]
        == "0xefb08999196366871f919fd0e138b3a7f30ee33e678df7cfaf203e25f0a4278"
    )
    assert signature_testnet["v"] == 28
...
def test_schedule_cancel_action():
    ...
    assert (
        signature_mainnet["s"]
        == "0x6557ac307fa05d25b8d61f21fb8a938e703b3d9bf575f6717ba21ec61261b2a0"
    )
    assert signature_mainnet["v"] == 27
    signature_testnet = sign_l1_action(wallet, action, None, 0, False)
    assert (
    ...
    assert (
        signature_testnet["s"]
        == "0x342f8ede109a29f2c0723bd5efb9e9100e3bbb493f8fb5164ee3d385908233df"
    )
    assert signature_testnet["v"] == 28
    ...
    assert (
        signature_mainnet["s"]
        == "0x16c685d63b5c7a04512d73f183b3d7a00da5406ff1f8aad33f8ae2163bab758b"
    )
    assert signature_mainnet["v"] == 28
    signature_testnet = sign_l1_action(wallet, action, None, 0, False)
    assert (
    ...
    assert (
        signature_testnet["s"]
        == "0x706c656b23428c8ba356d68db207e11139ede1670481a9e01ae2dfcdb0e1a678"
    )
    assert signature_testnet["v"] == 27
